...
class Revendedores extends model{
    public function cadastrar($nome, $cpf, $rg, $celular, $telefone, $email, $endereco, $bairro, $cidade, $cep, $estado){
        ...
        $sql->execute(array($nome, $cpf, $rg, $celular, $telefone, $email, $endereco, $bairro, $cidade, $cep, $estado));
    }

    public function getTotalRevendedores(){
        $sql = $this->db->prepare("SELECT id FROM revendedores");
        $sql->execute();
        $sql = $sql->rowCount();
        return $sql;
    }

    public function getRevendedores($pagina, $limite){
        $p = ($pagina - 1) * $limite;
        $sql = $this->db->prepare("SELECT * FROM revendedores ORDER BY id DESC LIMIT ".$p.",  ".$limite." ");
        $sql->execute();
        $sql = $sql->fetchAll();
        return $sql;
    }

    public function getRevendedor($id){
        $sql = $this->db->prepare("SELECT * FROM revendedores WHERE id = ?");
        $sql->execute(array($id));
        $sql = $sql->fetch();
        return $sql;
    }

    public function pesquisarRevendedores($termo, $pagina, $limite){
        $p = ($pagina - 1) * $limite;
        $sql = $this->db->prepare("SELECT * FROM revendedores WHERE nome LIKE ? OR email LIKE ? ORDER BY id DESC LIMIT ".$p.",  ".$limite." ");
        $sql->execute(array("%".strtolower($termo)."%", "%".strtolower($termo)."%"));
        $sql = $sql->fetchAll();
        return $sql;
    }

    public function pesquisaRevendedores($termo){
        $sql = $this->db->prepare("SELECT * FROM revendedores WHERE nome LIKE ? OR email LIKE ? ORDER BY id DESC");
        $sql->execute(array("%".strtolower($termo)."%", "%".strtolower($termo)."%"));
        $sql = $sql->fetchAll();
        return $sql;
    }

    public function getRevendedoresStatus($status){
        $sql = $this->db->prepare("SELECT * FROM revendedores WHERE status = ?");
        $sql->execute(array($status));
        $sql = $sql->fetchAll();
        return $sql;
    }

    public function getRevendedoresComStatus($status, $pagina, $limite){
        $p = ($pagina - 1) * $limite;
        $sql = $this->db->prepare("SELECT * FROM revendedores WHERE status = ? ORDER BY id DESC LIMIT ".$p.",  ".$limite." ");
        $sql->execute(array($status));
        $sql = $sql->fetchAll();
        return $sql;
    }

    public function editarRevendedor($id, $nome, $cpf, $rg, $celular, $telefone, $email, $endereco, $bairro, $cidade, $cep, $estado, $status){
        $sql = $this->db->prepare("UPDATE revendedores SET nome = ?, cpf = ?, rg = ?, celular = ?, telefone = ?, email = ?, endereco = ?, bairro = ?, cidade = ?, cep = ?, estado = ?, status = ? WHERE id = ?");
        $sql->execute(array($nome, $cpf, $rg, $celular, $telefone, $email, $endereco, $bairro, $cidade, $cep, $estado, $status, $id));
    }

    public function setStatus($id, $status){
        $sql = $this->db->prepare("UPDATE revendedores SET status = ? WHERE id = ?");
        $sql->execute(array($status, $id));
    }

    public function excluirRevendedor($id){
        $sql = $this->db->prepare("DELETE FROM revendedores WHERE id = ?");
        $sql->execute(array($id));
    }
